...
public class LendBook {
	...
	public void returnBooks(@RequestParam("readerId") int readerId, @RequestParam("bookId") int bookId, HttpServletResponse response) throws IOException{
		int res = lendInfoService.updateLendInfo(readerId, bookId);
		
		if(res == 1){
			System.out.println("还书成功");
			response.sendRedirect("bookResult.jsp?tab=3");
		}else {
			if(res == 2){
				System.out.println("逾期还书");
				response.sendRedirect("bookResult.jsp?tab=4");
			}else{
